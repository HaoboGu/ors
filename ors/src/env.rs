...
use anyhow::Result;
use crate::{log::{LoggingLevel, custom_logger}, call_ort, api::get_api, status::check_status};
...
impl Environment {
    ...
    pub(crate) fn env_ptr(&self) -> *const OrtEnv {
    ...
    fn new(name: String, log_level: LoggingLevel) -> Result<Environment> {
        ...
        if g_env_ptr.is_null() {
            ...
            let status = call_ort!(CreateEnvWithCustomLogger, logging_function,
                        logger_param,
                        log_level.into(),
                        cname.as_ptr(),
                        &mut env_ptr);
...
impl Drop for Environment {
    ...
    fn drop(&mut self) {
        ...
        //       can properly free the OrtEnv pointer.
        if Arc::strong_count(&G_ENV) == 2 {
            let env_ptr: *mut OrtEnv = *environment_guard.env_ptr.get_mut();
