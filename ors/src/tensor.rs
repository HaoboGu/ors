...
fn get_dimensions(type_info: *const OrtTensorTypeAndShapeInfo, dimension_cnt: u64) -> Vec<i64> {
...
fn get_dimension_count(type_info: *const OrtTensorTypeAndShapeInfo) -> u64 {
...
fn get_tensor_shape_element_count(type_info: *const OrtTensorTypeAndShapeInfo) -> u64 {
	let mut element_cnt = 0;
	let status = unsafe {
		get_api().GetTensorShapeElementCount.unwrap()(
			type_info,
			&mut element_cnt,
		)
	};
