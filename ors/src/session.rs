...
impl SessionBuilder {
	...
	pub fn build_with_model_from_file<P>(self, model_filepath: P) -> Result<Session> where 
    P: AsRef<Path> {
	...
	pub fn graph_optimization_level(
		...
		let status = call_ort!(SetSessionGraphOptimizationLevel, self.session_options_ptr, opt_level.into());
		check_status(status)?;
		Ok(self)
	}
